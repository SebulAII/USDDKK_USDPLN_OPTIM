...
class TestStrategy(backtrader.Strategy):
    ...
    def next(self):
        ...
        #wygładzanie
            ...
            if self.line.idx > self.warmup and len(self) > self.pip_av_days and len(self) > self.av_VIX_days:
                ...
                self.VIX = self.datas[4].close[0]
                self.carry1_no_vol = (((self.datas[2].close[0]  * ((self.datas[0].close[0])/self.datas[0].close[0]))))
                self.carry2_no_vol = (((self.datas[3].close[0]  * ((self.datas[1].close[0])/self.datas[1].close[0]))))
                self.carry_sum_daily = self.carry1_no_vol + self.carry2_no_vol
                ...
                if len(self) <= self.max_module_period and len(self) > self.av_VIX_days:
            ...
            if self.ticket is not None:
               ...
               # self.swap_by_profit = self.swapsSum*100+ self.test_daily_change_summary_report
